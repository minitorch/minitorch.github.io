...
The next element on the queue is the top arrow. Here we have an interesting result. We have a new arrow, but it corresponds to the same variable which is already in the queue. It is okay to have the variable twice. Or alternatively we can apply an optimization, and simply add it derivative to the derivative computed at the last step. This means there is only one arrow to process.
...
Here each of the red arrows is represented by an object :class:`minitorch.VariableWithDeriv`, that stores the Variable and its current derivative (what gets passed to :math:`d_{out}` in chain rule). Starting from the last arrow, which is passed in as an argument, backpropagate should run the following algorithm.
    ...
    2. a. If the variable is a leaf then add its final derivative (`_add_deriv`) and loop to (1).
       b. If the variables is not a leaf, run the chain rule the last function that created it. Pass in the with derivative of the variable as :math:`d_{out}`.
       c. Iterate through each non-constant variable in the chain rule. 
       d. Optional. If the variable is in the queue (check `.name`), add to its current derivative
       e. Otherwise, add to the queue.
